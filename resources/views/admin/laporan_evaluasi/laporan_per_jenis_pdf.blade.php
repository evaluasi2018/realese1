	...
	<table style="width: 100%; border-collapse: collapse;">
		...
		<tbody>
			@php
				$no=1;
			@endphp
			@foreach($export_per_jenis as $export)
				<tr id="tr">
					<td id="td" height="35">{{ $no }}</td>
					<td id="td" height="35">{{ $export->nm_dosen }}</td>
					<td id="td" height="35">{{ $export->nm_matkul }}</td>
					<td id="td" height="35">{{ $export->nm_prodi }}</td>
					<td id="td" height="35">{{ $export->nm_fakultas }}</td>
					<td id="td" height="35">{{ $export->kelas }}</td>
					<td id="td" height="35">{{ $export->id_jenis_indikator }}</td>
					<td id="td" height="35">{{ $export->totalnilai }}</td>
				</tr>
				@php
					$no++;
				@endphp
